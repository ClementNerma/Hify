...
pub fn analyze_file(path: &Path) -> Result<(TrackMetadata, TrackStrTags)> {
    ...
    let codec = match track.codec_params.codec {
        CODEC_TYPE_MP3 => TrackAudioCodec::MP3,
        CODEC_TYPE_FLAC => TrackAudioCodec::FLAC,
        // CODEC_TYPE_WAVPACK => TrackAudioCodec::WAV,
        CODEC_TYPE_AAC => TrackAudioCodec::AAC,
        CODEC_TYPE_VORBIS => TrackAudioCodec::VORBIS,
        CODEC_TYPE_OPUS => TrackAudioCodec::OPUS,
    ...
    let dur = match track.codec_params.n_frames {
        ...
        None => analyze_file_duration(format, track_id, time_base)?,
    };

    Ok((
        TrackMetadata {
            audio_codec: codec,
            duration_s: u32::try_from(dur.seconds)
                .context("Audio track is longer than 2^32-1 seconds!")?
                + u32::from(dur.frac > 0.5),
        },
