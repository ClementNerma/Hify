...
macro_rules! os_struct {
    ($pub:vis struct $struct_name:ident {
        ...
        impl $crate::server::opensubsonic::xml::XMLCompatible for $struct_name {
            #[allow(clippy::vec_init_then_push)]
            fn to_xml_element(&self) -> $crate::server::opensubsonic::xml::XMLElement {
                #[allow(unused_imports)]
                use $crate::server::opensubsonic::xml::{
                    XMLChildren,
                    XMLOptionalChildren,
                    XMLOptionalField,
                    XMLElement,
...
impl XMLFieldValue {
    fn serialize_to_xml(&self, out: &mut impl Write) -> std::fmt::Result {
        match self {
            Self::Bool(bool) => write!(out, "{bool}"),
            Self::SignedInt(int) => write!(out, "{int}"),
            Self::UnsignedInt(int) => write!(out, "{int}"),
            Self::Float(float) => write!(out, "{float}"),
            Self::StaticString(str) => out.write_str(&escape_xml_str(str)),
            Self::String(str) => out.write_str(&escape_xml_str(str)),
        }
    }
    ...
    fn serialize_to_json(&self, out: &mut impl Write) -> std::fmt::Result {
        match self {
            ...
            Self::UnsignedInt(int) => write!(out, "{int}"),
            Self::Float(float) => write!(out, "{float}"),
            Self::StaticString(str) => write!(out, "\"{}\"", escape_json_str(str)),
            Self::String(str) => write!(out, "\"{}\"", escape_json_str(str)),
        }
    }
}
...
impl<T: XMLCompatible> XMLChildrenRef<'_, T> {
    pub fn to_owned(&self) -> XMLChildren {
        match self {
            ...
            XMLChildrenRef::Multiple(items) => {
                XMLChildren::Multiple(items.iter().map(XMLCompatible::to_xml_element).collect())
            }
        }
    }
}
...
pub trait XMLOptionalChildren<T: XMLCompatible> {
    fn to_option(&self) -> Option<XMLChildrenRef<'_, T>>;
}
